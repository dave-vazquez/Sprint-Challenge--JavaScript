...
do { // bubble-sort from JS-I - (tempted to try another sorting algorithm, but seems silly for an array of length 10...)
  unsorted = false;
...
console.log('\nUniversities with \'Uni\' in the name:');
console.log(uni);
...
The zoos need a list of all their animal's names (names only, not scientific) converted to lower case.  Create a new array named lowerCase and map over each name to 
convert them all to lower case.  Log the result.

*/

const lowerCase = zooAnimals.map(animal => animal.animal_name.toLowerCase());

console.log('\nAnimal names lower-cased:')
...
*/
const populationTotal = zooAnimals.reduce((runningTotal, animal) =>  runningTotal + animal.population, 0);
console.log('\nTotal Animal Population:', populationTotal);
